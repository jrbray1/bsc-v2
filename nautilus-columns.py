...
# cp nautilus-columns.py /usr/share/nautilus-python/extensions/
# chmod a+x /usr/share/nautilus-python/extensions/nautilus-columns.py
...
# John Bray: Fix Icon View, re-write title, description and creator
        ...
        if os.path.isfile(path_to_video):
            ...
            for metadata in data['media']['track']:
                if metadata['@type'] == 'General':
                    ...
                    self._title = metadata.get('Title', _('Unknown'))
...
lens, in mm.')),
            ...
            FileManager.Column(name='max_aperture_value_column',
...
gain adjustment')),
            FileManager.Column(name='width_column',
                               attribute='width',
                               label=_('Width'),
                               description=_(
                                    'Image/video/pdf width (pixel/mm)')),
            FileManager.Column(name='height_column',
                               attribute='height',
                               label=_('Height'),
                               description=_(
        ...
        if file.get_mime_type().split('/')[0] in ('image'):
            ...
            try:
                ...
                try:
                    file.add_string_attribute(
                        'creator',
                        metadata.get_tag_string('Xmp.dc.creator'))
                except Exception:
                    file.add_string_attribute('creator', '')
                try:
                    file.add_string_attribute(
